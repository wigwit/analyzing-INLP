...
emb_tr, y_tr, num_tags_train = gold_train.from_sents_to_words('syn',train_emb)
...
## all in cpu right now
emb_dev, y_dev, num_tags_dev = gold_dev.from_sents_to_words('syn',dev_emb)

gold_test = DataProcessing('silver','test')
gold_test.bert_tokenize()
test_emb = gold_test.get_bert_embeddings('load')
print('done saving')

## all in cpu right now
emb_test, y_test, num_tags_test = gold_test.from_sents_to_words('syn',test_emb)

if torch.cuda.is_available():
	torch.cuda.empty_cache()
print(num_tags_train)
...
inlp_syn = INLPTraining(emb_tr,y_tr,num_tags_train)
...
with open('../data/pmb_silver/rm_ccg/train.pkl', 'wb') as file:
	pickle.dump([inlp_syn.embeddings, inlp_syn.output], file)

inlp_syn = INLPTraining(emb_dev,y_dev,num_tags_dev)
# inlp_syn = inlp_syn.to(device)
P,P_is,Ws=inlp_syn.run_INLP_loop(10)
print(f'the rank of P is :{np.linalg.matrix_rank(P)}')
for P_i in P_is:
	print(np.linalg.matrix_rank(P_i))
with open('../data/pmb_silver/rm_ccg/dev.pkl', 'wb') as file:
	pickle.dump([inlp_syn.embeddings, inlp_syn.output], file)

inlp_syn = INLPTraining(emb_test,y_test,num_tags_test)
# inlp_syn = inlp_syn.to(device)
P,P_is,Ws=inlp_syn.run_INLP_loop(10)
print(f'the rank of P is :{np.linalg.matrix_rank(P)}')
for P_i in P_is:
	print(np.linalg.matrix_rank(P_i))
with open('../data/pmb_silver/rm_ccg/test.pkl', 'wb') as file:
	pickle.dump([inlp_syn.embeddings, inlp_syn.output], file)
