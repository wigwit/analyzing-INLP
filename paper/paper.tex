...
We use the English gold standard of Parallel Meaning Bank v4.0 \citep{pmbData} to test the linear separability of the semantic and syntactic information in word-level BERT embeddings. This data set contains word-level semantic tags and NAME FEATURE USED FOR CCG DERIVATIONS %Fill this in
 that we parse using CITE CCG PARSER %Fill this in
 to develop word-level CCG tags, both of which are used to train the linear classifiers used for probing and evaluation. The gold-standard data set is comprised of XX words. %Fill this in
...
\begin{table*}[h]
    ...
    %& & & \\
    $v_{nosem}$ & 75 & $26.36\%$ & $7.85\%$ \\
    $v_{nosyn}$ & 160 & $4.21\%$ & $0.24\%$ \\
    $v_{syn-sem}$ & 0 & $31.99\%$ & $36.27\%$ \\
    $v_{sem-syn}$ & 0 & $29\%$ & $51.58\%$ \\
    $v_{syn*sem}$ & 0 & p$\%$ & p$\%$ \\
    $v_{sem*syn}$ & 0 & p$\%$ & p$\%$ \\
    \hline
    \hline
